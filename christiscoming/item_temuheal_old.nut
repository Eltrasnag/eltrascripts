...
function MakeDummyParent() {
	...
	hDummyParent = SpawnEntityFromTable("prop_dynamic_ornament", {
		...
		for (local ply; ply = Entities.FindByClassnameWithin(ply, "player", vOrigin, iPickupScanRadius);) {
			...
			if (ply.GetTeam() == TEAMS.HUMANS && ply.IsUsingActionSlot()) {
				...
				if (!"SeenItemTut" in plyscope || DEV == true) {
					printl("firsttime")
					ClientPrint(ply, Constants.EHudNotify.HUD_PRINTCENTER, "|| To use this item, press the >ACTION SLOT< key. To drop this item, duck, then press the >ACTION SLOT< key again. ||")
...
function ActiveThink() {
	...
	if (ButtonPressed(hOwner, IN_JUMP) && hOwner.IsUsingActionSlot() && (iPlayerFlags & FL_ONGROUND)) {
		DropWeapon()
		return
	}
