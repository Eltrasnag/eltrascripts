...
HitboxModelName <- BRUSHMODELS["chica_hitbox"]


hBossBase <- SpawnEntityFromTable("base_boss", {
		targetname = "chica_hurter",
		model = BRUSHMODELS["chica_hitbox"], // can only safely do this in onpostspawn
		health = 99999999,
		maxhealth = 99999999,
	})
...
iFollowSpeed <- 64 // How fast should we follow player when fully accelerated
...
UniqueName <- "NPC_chica_"+UniqueString()
...
function OnPostSpawn() {
	...
	// 	maxhealth = 99999,
	// })
	SetParentEX(hVisualWeapon, self, "anim_attachment_RH")
	...
	hBossBase.SetAbsOrigin(self.GetOrigin()+bboxoffset)
...
function SleepThink() {
	...
	if (Time() > iNextIdleActionTime) {
		...
		if (bIsWalking == true) { // entering idle stance
			bIsWalking = false
			SetAnimation(hVisual, "Idle_Angry_SMG1")
			// printl("stop walk")
		} else { // wandering around
			WanderTries = 0
			FindWanderTarget()
			SetAnimation(hVisual, "Walk_aiming_all")
			bIsWalking = true
			// printl("start walk")
		}
	}
...
function ActiveThink() {
	...
	self.SetAbsAngles((QuaternionSlerp(vAngles.ToQuat(), (RotateOrientation(AngToPlayer, QAngle(0,0,0))).ToQuat(), 0.07)).ToQAngle());
...
function Sleep() {
	...
	SetAnimation(hVisual, "idle_angry")


}

function RandomScream() {
	local screamstr = "eltra/72hr/chica/hurt"+RandomInt(1,4).tostring()+".mp3"

	// PlaySoundEX(screamstr, self.GetOrigin(), 10, RandomInt(95, 105), self)
	// PlaySound(screamstr, self.GetOrigin())
...
function Wake() {
	...
	SetAnimation(hVisual, "RunAIMALL1")
...
function Death() {
	// hVisual.BecomeRagdollOnClient(self)
	AddThinkToEnt(self, "")
	if (ValidEntity(hBossBase)) {
		hBossBase.Kill()
	}
	if ("XMAS_chicaS" in getroottable()) {
		XMAS_chicaS++
	}
	QFireByHandle(hVisual, "color", "255 255 255")
	hVisual.BecomeRagdollOnClient(Vector(self.GetForwardVector()  * RandomInt(100,540) + Vector(0,0, RandomInt(300,830))))
...
function ShootWeapon() {
	// local trace = QuickTrace(self.EyePosition(), self.EyePosition() + self.EyeAngles() * 2000, MASK_ALL)
	// local wandersound = "npc/turret_floor/shoot"+RandomInt(1,3)+".wav"
	// PrecacheSound(wandersound)
	// EmitAmbientSoundOn(wandersound, 1, 125, RandomInt(96,106), self)
	// local flash = Spawn("light_dynamic", {
	// 	brightness = RandomInt(20,40),
	// 	distance = RandomInt(125,96),
	// 	origin = hVisualWeapon.GetOrigin()
	// })
	// QFireByHandle(flash, "Kill","", 0.1)
	// if (trace.hit && trace.enthit.GetClassname() == "player") {
	// 	local ply = trace.enthit
	// 	ply.TakeDamage(RandomInt(5,14), Constants.FDmgType.DMG_BULLET, self)
	// }
}