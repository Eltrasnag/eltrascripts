...
// HitboxModelName <- BRUSHMODELS["dog_hitbox"]


hBossBase <- SpawnEntityFromTable("base_boss", {
		targetname = "doghurtbox",
		model = "models/error.mdl", // can only safely do this in onpostspawn // was htis lie
		health = 99999999,
		maxhealth = 99999999,
	})
...
iFollowSpeed <- 500 // How fast should we follow player when fully accelerated
...
UniqueName <- "NPC_dog_"+UniqueString()
...
function OnPostSpawn() {
	...
	// 	maxhealth = 99999,
	// })
	self.KeyValueFromString("targetname", "dog")
	hBossBase.SetModelSimple(BRUSHMODELS["dog_hitbox"])
...
function ActiveThink() {
	...
	if (!ValidEntity(hTarget) || !hTarget.IsAlive() || !hTarget.GetTeam() == TEAMS.HUMANS || iObstacles > iMaxObstacles || Time() >= iNextRetargetTime) { // verify if target is valid, if not then retarget
		if (!Retarget()) {
	...
	self.SetAbsAngles((QuaternionSlerp(vAngles.ToQuat(), (AngToPlayer).ToQuat(), iTurnAccel)).ToQAngle());
...
function Retarget() { // previous method was flawed and would always return the same player
	...
	local vOrigin = self.GetOrigin() // what is the point of this
