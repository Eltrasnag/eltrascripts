// by ficool2
// edited by eltra go to https://github.com/ficool2/vscript_vehicle if you want to use this in a map
...
{
	if (!can_enter || driver || player.GetTeam() == TEAMS.SPECTATORS)
		return;
	...
	ClientPrint(driver, Constants.EHudNotify.HUD_PRINTCENTER, "Press 'DUCK' to exit the vehicle")
	driver.AcceptInput("SetParent", "!activator", vehicle, vehicle);
	driver.SetForcedTauntCam(1)
	...
	QFireByHandle(vehicle, "CallScriptFunction", "EnableExit", 1.0, null, null);
}

function Exit(dead, teleport = false)
{
	if (driver)
	{
		if (!dead)
	...
	can_enter = false;
	QFireByHandle(vehicle, "CallScriptFunction", "EnableEnter", 1.0, null, null);
}
...
function Think()
{
	...
	if (driver)
	{
		...
		if (can_exit)
		{
			if ((buttons & IN_DUCK) || driver.IsUsingActionSlot())
				Exit(false, true);
		}
...
::VehicleEvents =
{
	...
	OnGameEvent_scorestats_accumulated_update = function(params)
	{
		for (local vehicle; vehicle = Entities.FindByClassname(vehicle, "prop_vehicle_driveable");)
			...
			if ("driver" in this && ValidEntity(driver)) {
				local driver_scope = driver.GetScriptScope();
				if ("vehicle" in driver_scope && "vehicle_scope" in driver_scope) {
					driver_scope.vehicle = null;
					driver_scope.vehicle_scope = null;
	...
	OnScriptHook_OnTakeDamage = function(params)
	{
		...
		if (victim.GetClassname() == "prop_vehicle_driveable")
		{
			...
			if (driver && driver.GetTeam() != params.inflictor.GetTeam())
			{
				...
				if (driver.GetTeam() == TEAMS.ZOMBIES) { // z knockback

					victim.SetPhysVelocity(victim.GetPhysVelocity() + params.damage_force)
