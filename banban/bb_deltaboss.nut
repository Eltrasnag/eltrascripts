...
// LASERINSURGENCY - GRANDMA (THE DELTARUNE BOSS) [PORTED TO TF2] {RUINED IN BANBAN}

// CURRENT ISSUES: PLAYERS CANNOT DAMAGE BOSS IN TF2!
...
MDL_GRANDMA <- "models/eltra/funy/reimu.mdl"
...
function StartIntro() {
	...
	MapSayArray(["So you want to escape from here.....", "you wat to ekcape from my",GetMapName().toupper(),"Fuck you"],"Reimu")
...
::DeltifyPlayer <- function(hPlayer) {
	// hPlayer.SetCollisionGroup(Constants.ECollisionGroup) // something something skial zombie escape
	// hPlayer.DisableDraw()
	QFireByHandle(hPlayer,"Addoutput","rendermode 10")

	local tfstupid = SpawnEntityFromTable("info_particle_system", {
	...
	SetParentEX(tfstupid,hPlayer)
	// SetParentEX(hSoul,tfstupid)
...
function Granimate(strAnimation = "idle01") { // reimu has no animations
...
function StartEncounter() {
	...
	hMusic = MakeMusicTF("#"+SND_BOSSMUSIC)
...
function BossThink() {
	...
	switch (iGPhase) {
		case GRANDMA_PHASES.PHASE_INTRO:
			hBossText.KeyValueFromString("message", "-= REIMU =-")
			break;
	...
	for (local hPlayer; hPlayer = Entities.FindByClassname(hPlayer, "player");) {
		if (ValidEntity(hPlayer)) {
			...
			if (hPlayer in tDeltaPlayers) {
				local soul = tDeltaPlayers[hPlayer]
				soul.KeyValueFromVector("origin",vPlayerOrigin)
...
function Phase2() {
	...
	hMusic.Kill()
	hMusic = MakeMusicTF("#"+SND_PHASE2MUSIC)
	hMusic.AcceptInput("PlaySound", "", null, null)
...
function OnTakeDamage(activator) { // grandma takes dmg ouch
	printl("owowooww")
	if (bActive && activator.IsPlayer() && iGPhase == GRANDMA_PHASES.PHASE_MAIN) { // ensure that it is player doing damage, not her own attack
		iHealth--
...
function DoBossScale() {
	...
	iHealth = clamp(BOSS_SCALE_MULT * (iPlayerCount / 16.0), 40, BOSS_MAX_HEALTH)
}
