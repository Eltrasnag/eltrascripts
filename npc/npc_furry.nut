...
HitboxModelName <- BRUSHMODELS["furry_hitbox"]


hBossBase <- Spawn("base_boss", {
		targetname = "Ferven",
		model = BRUSHMODELS["furry_hitbox"], // can only safely do this in onpostspawn
		health = 99999999,
		maxhealth = 99999999,
	})
...
UniqueName <- "NPC_FURRY_"+UniqueString()
...
function ActiveThink() {
	...
	self.SetAbsAngles((QuaternionSlerp(vAngles.ToQuat(), AngToPlayer.ToQuat(), easeinBack(iCurAccel))).ToQAngle());
