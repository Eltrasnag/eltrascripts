...
HitboxModelName <- BRUSHMODELS["bottiger_hitbox"]
...
UniqueName <- "NPC_BOTTIGER_"+UniqueString()
...
function OnPostSpawn() {
	...
	hBossBase = SpawnEntityFromTable("base_boss", {
		targetname = "bottiger",
		model = BRUSHMODELS["bottiger_hitbox"], // can only safely do this in onpostspawn
		health = 99999999,
		maxhealth = 99999999,
	})
...
function ActiveThink() {
	...
	if (time >= iNextAggroSoundTime) {
		...
		// PlaySoundEX(SF_AGGRESSION, vOrigin, 10, RandomInt(95,105), self) // shut the fuck up please
	...
	// if we are about to go over a cliff then turn, if we still can't find a safe path then retarget
